...
dt.data <- data.table(read.csv("/Users/garidor/Desktop/bandits-rl-project/results/free_obs_raw_results/free_obs_experiment_raw_results_6.csv", header=T, stringsAsFactors=T))
...
ggplot(dt.data_subset, aes(x=as.factor(Memory.Size), y=Market.Share.for.P1, color=as.factor(Time.Horizon))) + 
  ...
  stat_summary_bin(fun.y=mean, geom="point", shape=18, size=5) +
