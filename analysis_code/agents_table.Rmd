...
dat <- read.csv(file=paste(WORKING_PATH, "/results/tournament_raw_results/tournament_experiment_full_sim_with_realizations_raw.csv", sep=""))
dat_3 <- dat
...
warm_starts <-  sort(as.list(unique(dat_3['Warm.Start']))$Warm.Start, decreasing=FALSE)
      ...
      for (k in 1:length(alg_pairs)) {
        if (k %% 2 == 0) { next }
          ...
          if (q == 1) {
            ...
            filtered_dat <- filter(dat_10, P1.Alg == p1alg & Warm.Start == start & P2.Alg == p2alg & Prior == prior & Agent.Alg == agent_alg & Time.Horizon == time)
          }
