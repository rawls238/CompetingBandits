...
dat <- read.csv(file=paste(WORKING_PATH, "/results/tournament_raw_results/tournament_experiment_heavy_tail_many_sim_raw.csv", sep=""))
dat_3 <- dat
...
memory_sizes <- sort(as.list(unique(dat_3['Memory.Size']))$Memory.Size, decreasing=TRUE)
...
for (prior in priors) {
  ...
  for (q in 1:1) {
    for (i in 1:length(agent_algs)) {
      ...
      for (k in 1:length(alg_pairs)) {
        if (k %% 2 == 0) { next }
          ...
          if (q == 1) {
            ...
            K <- 10
            filtered_dat <- filter(dat_10, P1.Alg == p1alg & P2.Alg == p2alg & Memory.Size == memory & Prior == prior & Agent.Alg == agent_alg & Time.Horizon == time)
          }
      ...
      tab <-xtable(results, caption=paste("Results for", "t=",time, prior, "Memory=", memory, "K=", K), row.header='Entrant', col.header='Incumbent')
      print(tab, type="html")
    }
      }
  }
