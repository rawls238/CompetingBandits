...
dat <- read.csv(file=paste(WORKING_PATH, "/results/tournament_raw_results/tournament_experiment_multiple_haystack_2_raw.csv", sep=""))
...
for (prior in priors) {
    ...
    for (i in 1:length(agent_algs)) {
      ...
      tab <-xtable(results, caption=paste("Results for", "t=",time, prior, "Memory=", memory), row.header='Entrant', col.header='Incumbent')
      print(tab, type="html")
  }
  }
}
