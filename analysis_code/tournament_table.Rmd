...
dat <- read.csv(file=paste(WORKING_PATH, "/results/tournament_raw_results/tournament_experiment_testing_prelim_raw.csv", sep=""))
      ...
      print_breaks <- FALSE
        ...
        for (k in 1:length(algs)) {
          for (l in 1:length(algs)) {
            p1alg = algs[k]
            p2alg = algs[l]
            filtered_dat <- filter(dat, P1.Alg == p1alg & P2.Alg == p2alg & Warm.Start == 100 & Prior == prior & Agent.Alg == agent_alg & Time.Horizon == time)
