...
\swedit{To answer these questions, we isolate the effects from the two
  ...
  the entrant \gaedit{so that the incumbent's reputation score is only set by the warm start periods and not any of the incumbency periods}. In the second experiment (called \emph{information
...
\begin{finding}
\textit{Maintaining either the data or reputation advantage alone still allows the incumbent \gaedit{to retain a large share of the market compared to the share they get under permanent duopoly}. \swcomment{too vague. what does significant mean?}\\ \indent When the incumbent plays $\TS$, the data advantage serves as a larger barrier to entry compared to reputation, but when the incumbent plays $\DEG$ or $\DG$ there is no substantial difference between the two barriers.}
\end{finding}
...
\gadelete{Though the setup is purely experimental, it is nonetheless interesting to look at if the same strategies remain best responses in this setting compared to the setting where both data and reputation are retained. We find that the best responses remain the same for the Heavy Tail instance, but for the Uniform instance they change where $\TS$ is weakly dominant for the incumbent under the reputation erased treatment but not under the information erased treatment.}
