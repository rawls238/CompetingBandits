...
\swedit{Each firm aims to maximize its market share---the number of agents coming to them.}
...
\swedit{For any single firm, we say that algorithm 1 \emph{weakly
    dominates} algorithm 2 if algorithm 1 never provides a lower
  market share than 2 against all opposing strategies. An
  algorithm is a weakly dominant strategy for a firm if the algorithm
  weakly dominates all other algorithms.}
  \gaedit{Since the algorithms that involve exploration have larger deployment costs, we break any indifference towards algorithms that do not involve exploration (i.e. if \DG and \DEG provide the same market share, the firm will play \DG).}
\swcomment{please use the above terminology}
