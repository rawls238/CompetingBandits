...
\swdelete{We break indifference between exploration algorithms and
  ...
  follows:} \swcomment{deleted the discussion of tie-breaking since it
  feels out of place.}
...
\end{center}
The conditions on $\TS$ being dominant under the temporary monopoly are that the incumbent is a temporary monopoly for sufficiently many periods.\swcomment{vague here. }
}
\end{finding}
...
\xhdr{Permanent Duopoly} \swdelete{Permanent duopoly corresponds to the model
described in section 2 where $X = 0$ so that both firms enter the
market simultaneously. What algorithms are firms incentivized to
deploy? We report the average market share taken over the $N$
simulations for a set of exogenous values of $T_0$. Additionally, we
report the mean and median of the effective end of game.}
\swedit{

  Since in this case both firms enter at the same time, they will both
  get an expected market share of $1/2$ whenever they deploy the same
  algorithms. 

To see what algorithms are the firms incentivized to deploy, we look at the average market
share taken over the $N$ simulations for different values of warm-up length $T_0$.
...
Even for large warm start, $\DG$ is better than $\DEG$ under Heavy Tail \footnote{We demonstrate this holds even for $K = 3$ in the supplementary material}
\end{puzzle}
