...
\footnote{\asedit{Reputation scores already introduce some noise into users' choices. However, the amount of noise due to this channel is typically small, both in our simulations and in practice, because reputation signals average over many datapoints.}}
 ...
 We confirm the theoretical intuition that better algorithms prevail if the parameter -- essentially, the expected number ``random users" -- is sufficiently large. However, we find that this effect is negligible for some smaller but still ``relevant" parameter values.
...
%
mean reputation, most connected to our intuition, is sometimes bad predictor in competition (see Sections~\ref{sec:isolation} and~\ref{sec:revisited});
%
mathematical tools from regret-minimization would only produce ``asymptotic" results, which does not seem to suffice.
We believe that resolving first-order theoretical questions would not add much value to this paper, because of \cite{CompetingBandits-itcs18}. We are in realm of ``asymptotic" theorems about stylized Economic models, which provide some mathematical intuition but no direct bearing on the world. But \cite{CompetingBandits-itcs18} already has an elaborate set of such results, with similar high-level conclusions. So, one would need to go beyond the first-order conclusions to add value.}
