...
the greedy algorithm, because it is easier to deploy in practice. Implicitly, users have no ``outside option": the service provided is an improvement over not having it (and therefore the monopolist is not incentivized to deploy better learning algorithms). This is plausible with free ad-supported platforms such as Yelp or Google.}}\footnote{\gaedit{The most common measures of market ``competitiveness" such as the Lerner Index or the Herfindahl-Hirschman Index of a market rely on ex-post observable attributes of a market such as prices or market shares \cite{tirole1988theory}. However, since we have no prices and market shares are an endogenous element of our model, we simply define ``competitiveness" in terms of the number of firms and timing of entry into the market.}}
...
\footnote{\asedit{Reputation scores already introduce some noise into users' choices. However, the amount of noise due to this channel is typically small, both in our simulations and in practice, because reputation signals average over many datapoints.}} We confirm the theoretical intuition that better algorithms prevail if the \gaedit{expected number of ``random" users is sufficiently large.}\gadelete{parameter -- essentially, the expected number ``random users" --is sufficiently large.} However, we find that this effect is negligible for some smaller but still ``relevant" parameter values.
...
\asedit{While amenable to simulations, our model appears difficult to analyze. This is for several reasons:
%
intricate feedback loop from performance to reputations to users to performance;
%
mean reputation, most connected to our intuition, is sometimes \gaedit{a} bad predictor in competition (see Sections~\ref{sec:isolation} and~\ref{sec:revisited});
%
mathematical tools from regret-minimization would only produce ``asymptotic" results, which do not seem to suffice. We believe that resolving first-order theoretical questions would not add much value to this paper, because of \cite{CompetingBandits-itcs18}.\gadelete{We are in \gaedit{the} realm of ``asymptotic" theorems about stylized economic models, which provide some mathematical intuition but no direct bearing on the world. But \cite{CompetingBandits-itcs18} already has an elaborate set of such results, with similar high-level conclusions. So, one would need to go beyond the first-order conclusions to add value.}}
