...
\OMIT{This occurs because the lack of initial competition allows the incumbent to gather data on actions that may be harder to gather in competition (due to reputational consequences) but that are useful for learning and maximizing long-run reputation.}


\OMIT{ %%%%%
\gadelete{In this scenario, the incumbent firm enjoys both a ``data advantage" and a ``reputational advantage". We further investigate which of the two is a stronger barrier to entry. We find that both are strong barriers on its own: removing either one substantially helps the late entrant. Further, the data advantage tends to be a stronger barrier when the incumbent commits to a more advanced bandit algorithm.}
} %%%%%%
...
Our work is related to a longstanding economics literature on competition vs. innovation, \eg \cite{Schumpeter-42,barro2004economic,Aghion-QJE05}. While this literature focuses on R\&D costs of innovation, ``reputational costs" seem new and specific to exploration. \asedit{First-mover advantage phenomenon is well-studied in economics, \eg see \cite{kerin1992first}.}
