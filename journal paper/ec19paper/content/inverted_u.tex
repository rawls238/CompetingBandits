...
\xhdr{Simultaneous entry duopoly.}
...
\textit{Under simultaneous entry duopoly:
\begin{itemize}
...
\end{tabular}
\caption{{\bf Simultaneous entry duopoly}, for the Uniform MAB instance. Semantics are the same as in Table \ref{sim_table}.}
...
Finally, the middle level of competition, ``incumbent" in the temporary monopoly creates strong incentives for \TS. In stylized form, this relationship is captured in Figure~\ref{fig:inverted-U-expts}.%
...
\begin{finding}
\textit{When all firms deploy \DG, and start at the same time, welfare is weakly decreasing as the number of firms \gaedit{increases}}
\end{finding}
