...
\begin{finding}
\textit{
Mean reputation trajectories do not \gadelete{suffice to }explain the outcomes under competition.}
\end{finding}
...
We see that the ``naive" algorithms $\DynamicGreedy$ and $\DynamicEpsGreedy$ have a bi-modal reputation distribution, whereas $\Thompson$ does not. The reason is that for this MAB instance, $\DynamicGreedy$ either finds the best arm and sticks to it, or gets stuck on the bad arms. In the former case \DynamicGreedy does slightly better than $\Thompson$, and in the latter case it does substantially worse. However, the mean reputation trajectory \gadelete{may fail}\gaedit{fails} to capture this complexity since it \gadelete{simply }takes average over different \MRVs. This \gaedit{is}\gadelete{may be} inadequate for explaining the outcome of the duopoly game, given that the latter is determined by a \gadelete{simple} comparison between the firm's reputation scores.
