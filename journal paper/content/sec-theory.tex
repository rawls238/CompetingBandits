...
\xhdr{Assumptions.} We make two mild assumptions on the prior. First, each arm $a$ can be the best:
...
\BIR_i(n) := \E\sbr{ n\cdot \max_{a\in A} \mu_a - \rew_i(n)}.
\end{align}
We are primarily interested in how \BIR scales with $n$: typically, how fast \BIR decreases with $n$. We treat $K$, the number of arms, as a constant unless specified otherwise. 
...
The competition game has a unique Nash equilibirium: both principals choose \DynGreedy. \asedit{This holds even if the principals can only choose from restricted strategy sets which include \DynGreedy.}
\end{corollary}
...
\asedit{We'd like to use Theorem~\ref{thm:random-clean} to conclude that a (much) better algorithm prevails in equilibrium. Indeed, this holds if the principals can only choose from a finite set of MAB algorithms.}
...
The modified algorithm, called the \emph{greedy modification} of \alg  with \emph{mixing parameter} $p\in (0,1)$, is defined as follows. Suppose \alg deviates from \DynGreedy starting from some (local) step $n_0$. Then
\begin{enumerate}
...
\newcommand{\BIRgr}{\BIR^{\texttt{gr}}}


Next, let us argue that \alg[2] is monotone. We spell out the proof for the sake of intuition. For each $n\in\N$, let $M_n$ be the number of times \alg[1] is invoked in the first $n$ steps of \alg[2], and let $\BIRgr_1(n)$ be the $\BIR$ of the ``greedy choice" after $n-1$ steps of \alg[1]. Let 
    $\mathtt{reg}_2(n) = n\cdot \max_a \mu_a - \rew_2(n)$ 
be the (frequentist) instantaneous regret of \alg[2]. Then by definition of the greedy modification it holds that 
\begin{align}\label{eq:random-BIR}
\E\sbr{ \mathtt{reg}_2(n) \mid M_n = m}
    &= (1-p) \cdot \BIR_1(m) + p \cdot \BIRgr_1(m) .\\
\BIR_2(n)
    &= \E\sbr{ (1-p) \cdot \BIR_1(M_n) + p \cdot \BIRgr_1(M_n) }.
\end{align}
Now, monotonicity of $\BIR_2$ follows from monotonicity of $\BIR_1(\cdot)$ and $\BIRgr_1(\cdot)$. The former is assumed, and the latter is the monotonicity-in-information property of the ``greedy step" (see \ascomment{pointer} for a precise formulation and proof).
...
\asedit{These conclusions hold even if the principals' strategy sets are any collections of monotone MAB algorithms that are closed under greedy mixing.}%
\footnote{That is, the strategy set of each principal consists of some (but not necessarily all) monotone MAB algorithms, and contains all greedy mixtures of all algorithms in this set.}
\end{corollary}


\ascomment{Steven: I revised this example to clarify what it is supposed to mean. But the conclusion can't be right: "greedy modification" can only improve the BIR at step $n_0$! Perhaps what we derive is that
$\BIR_2(n)>\BIR_1(n)$ starting from $n=n_0+1$? }

For the sake of intuition, let us show how \alg[1] can be a better algorithm than its greedy modification, in the sense of having a strictly smaller \BIR.

\begin{example}
Consider \alg[1] and \alg[2] as in Theorem~\ref{thm:random-greedy}.
Let $\BIRgr_1(n)$ be the \BIR of the ``greedy choice" after $n-1$ steps of \alg[1]. Assume, for some absolute constants $\beta,\gamma>0$ and $c>1-\gamma$, that
\begin{align}\label{eq:cor:random-greedy-ex-assumption}
 \tfrac{1}{c}\; \BIRgr_1(n) \geq \BIR_1(n)= \beta\cdot n^{-\gamma}
...
\begin{align}\label{eq:random-BIR}
\BIR_2(n)
    &= \Ex{m\sim (n_0-1)+\term{Binomial}(n-n_0+1,1-p)}{(1-p) \cdot \BIR_1(m) + p \cdot \BIRgr_1(m)}.
\end{align}
...
Assume \SoftMaxRandom response function. Consider the \FiniteGame in which all feasible algorithms satisfy $\BReg(n)\to \infty$.%
\footnote{$\BReg(n)\to \infty$ is a mild non-degeneracy condition.
\ascomment{TODO: can we actually prove it? Assume Bernoulli priors, discrete prior, reduce to Lai-Robbins.}}
...
Many/most MAB algorithms are parameterized by the time horizon $T$, and their regret bounds usually include $\polylog(T)$. In particular,  a regret bound for \BIR is
\begin{align}
...
Accordingly, BIR-dominance can be redefined as follows: there exists a number $T_0$ and a function $n_0(T)\in \polylog(T)$
such that
