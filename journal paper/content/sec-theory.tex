...
For the sake of intuition, let us give a concrete example how the modified algorithm \alg[2] may have a larger \BIR compared to the original algorithm \alg[1], even though it performs exploration at a slower rate. Let $\BIRgr_1(n)$ be the \BIR of the ``greedy choice" after $n-1$ steps of \alg[1]. Assume 
...
\end{align}
for some absolute constants $\beta,\gamma>0$ and $c>1-\gamma$. We will derive
\begin{align}\label{eq:cor:random-greedy-ex-conclusion}
\BIR_2(n)
    >\alpha\cdot \BIR_1(n),
    \text{where} \quad
...
Indeed, by definition of the modified algorithm we have that
