...
\ascomment{some intro para goes here.}
...
We need to express a property that \alg[1] eventually catches up and surpasses \alg[2], even if initially it receives only a fraction of traffic. We assume that both algorithms run indefinitely and do not depend on the time horizon $T$; call such algorithms \emph{$T$-oblivious}. In particular, their \BIR does not depend on the time horizon $T$ of the game.  Then this property can be formalized as:
...
To state a game-theoretic corollary of Theorem~\ref{thm:random-clean}, we restrict the set of feasible MAB algorithms:
...
\ascomment{explain why, cite smth?}} 
