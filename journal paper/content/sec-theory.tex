...
\Pr\sbr{ \alg[1](n)\neq \alg[2](n)} >0,
\end{align}
where $\alg[i](n)$ is the arm chosen by \alg[i] at local step $n$.

\ascomment{TODO: we used to have a much more cumbersome definition (commented out), and we need it back! because, what if we have two copies of the same randomized algorithm, this eqn will hold!}

\ascomment{TODO: define "deviate starting from" as just "this is the first $n_0$ when the deviation happens.}
...
\ascomment{This whole remark is supposed to add intuition, but it seems pretty hard to parse ... :(}

\begin{remark}
The modified algorithm performs exploration % ingests information
at a slower rate. For the sake of intuition, let us argue how this may translate  into a larger \BIR compared to the original algorithm. Let  $\BIR'_1(n)$ be the \BIR of the ``greedy choice" after $n-1$ steps of \alg[1]. Then
...
for some $c>1-\gamma$.
\ascomment{Steven,Alex: not clear why this holds ...}
