...
\xhdr{Notation.} Let $\rew_i(n)$ denote the agent's realized reward observed by principal $i$ at local step $n$. Put differently, it is the reward collected by algorithm $\alg[i]$ in this local step. For a global round $t$, let $n_i(t)$ denote the number of global rounds before $t$ in which principal $i$ is chosen. Note that
...
\begin{definition}
One MAB algorithm \emph{deviates} from another at (local) step $n$ if there is a set $H$ of histories over the previous local steps such that both algorithms lead to $H$ with positive probability, and choose different distributions over arms given any history $h\in H$. We say $\alg$ deviates from $\alg'$ \emph{starting from} step $n_0$ if $n=n_0$ is the smallest step when this happens.
\end{definition}
