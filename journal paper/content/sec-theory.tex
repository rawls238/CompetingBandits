...
\end{align}
We are primarily interested in how \BIR scales with $n$: typically, how fast \BIR decreases with $n$. We treat $K$, the number of arms, as a constant unless specified otherwise. Intuitively, (much) better MAB algorithms tend to have a (much) smaller BIR, see Appendix~\ref{app:examples} for more background.
...
\ascomment{This whole blurb before the corollary is new.}
...
\asedit{These conclusions hold even if each principal's strategy set is a subset of monotone MAB algorithms that is closed under the greedy modification.}%
...
\footnote{$\BReg(n)\to \infty$ is a mild non-degeneracy condition, see 
\ascomment{insert pointer}.}
