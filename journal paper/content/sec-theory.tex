...
\xhdr{Notation.} Let $\rew_i(n)$ denote the \asedit{agent's realized reward observed by} principal $i$ at local step $n$. \asedit{Put differently, it is the reward collected by algorithm $\alg[i]$ in this local step.} For a global round $t$, let $n_i(t)$ denote the number of global rounds before $t$ in which principal $i$ is chosen. Note that
...
\xhdr{MAB algorithms.}
\asedit{We consider two baseline algorithms. The main one,} called  \DynGreedy, chooses an arm $a$ with the largest posterior mean reward $\E[\mu_a \mid \cdot ]$ given all information currently available to the algorithm. \asedit{A more primitive baseline,} called \StaticGreedy, chooses an arm $a$ with the largest prior mean reward $\E[\mu_a]$, and uses this arm in all rounds.
...
\begin{definition}
\asedit{One MAB algorithm \emph{deviates} from another} at (local) step $n$ if there is a set $H$ of histories over the previous local steps such that both algorithms lead to $H$ with positive probability, and choose different distributions over arms given any history $h\in H$. \asedit{We say $\alg$ deviates from $\alg'$} \emph{starting from} step $n_0$ if $n=n_0$ is the smallest step \asedit{when this happens.}
\end{definition}
...
The \HardMax model is very sensitive to tie-breaking between the principals. For starters, if ties are  broken deterministically in favor of principal $1$, then principal 1 can get all agents no matter what the other principal does, simply by using \StaticGreedy.
...
We'd like to use Theorem~\ref{thm:random-clean} to conclude that a (much) better algorithm prevails in equilibrium. We consider a version of the competition game in which the principals are restricted to choosing from a given set of MAB algorithms; the algorithms in this set are called \emph{feasible}.
...
\asedit{Note that we make no positive prediction if one MAB algorithm is only slightly better than the other. Next we show that such predictions are essentially impossible.}
...
\asedit{We argue below that \alg[1] can be a better algorithm than \alg[2]. This is consistent with Theorem~\ref{thm:random-clean}, since the BIR-dominance condition does not hold.}
...
Now, let us show how \alg[1] can be a better algorithm than its greedy modification \alg[2]. We need to assume that \alg[1] is (much) better than the greedy step \alggr if the latter is computed on a fraction of the data. Formally, we posit a convex function $f(\cdot)$ and $m_0>0$ such that
\[ \BIRgr(n) \geq f(n)
    \quad\text{and}\quad
...
\asedit{We also use \refeq{eq:BIR-modification} to argue that} \alg[2] is \bmonotone. This follows from \eqref{eq:BIR-modification} since both \alg[1] and \alggr are \bmonotone. The latter follows from a ``monotonicity-in-information" property of the ``greedy step": it can only get better with more information, see Lemma~\ref{lm:MII}.
...
Recall that by Theorem~\ref{thm:random-clean} \DynGreedy cannot be played in equilibrium when it is BIR-dominated by some feasible, $T$-oblivious algorithm and the time horizon $T$ is large enough. \asedit{Also recall that pure and mixed Nash equilibria are the same in our model.}

\asedit{Corollary~\ref{cor:random-greedy} holds more generally: it suffices to assume that feasible algorithms are \bmonotone and closed under the greedy modification. The latter means that the greedy modification of any feasible algorithm is also feasible, for any mixing parameter $p$.}
...
We frame our contributions in terms of the relationship between \competitiveness (as expressed by the ``hardness" of the agents' response function $\respF$),
...
Let us zoom in on the symmetric  \HardMaxRandom model. Competitiveness within this model are controlled by the baseline probability $\eps_0 = \respF(- 1)$, which goes smoothly between the two extremes of \HardMax ($\eps_0=0$, tough competition) and the uniform choice ($\eps_0=\tfrac12$, no competition). 
\asedit{Recall} % For clarity, we assume 
that principal's utility is the number of agents.
