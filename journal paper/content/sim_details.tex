...
\footnote{\asedit{In each round $t$, \Thompson computes a Bayesian posterior on $\mu_a$ for each arm $a$ and draws an independent sample $\tilde{\mu}_{a,t}$ from this posterior; it chooses an arm which maximizes $\tilde{\mu}_{a,t}$.
    \newline\indent 
