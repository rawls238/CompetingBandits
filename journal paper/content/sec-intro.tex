...
\xhdr{Our model: competition game.} We consider a stylized duopoly model in which two firms (\emph{principals}) compete for users (\emph{agents}). Principals compete on quality rather than on prices, and engage in exploration in order to learn which actions lead to high quality products. A new agent arrives and chooses a principal according to some decision rule \gaedit{which depends on the principal's exploration strategy}. The principal selects an action which affects the quality of service provided to this agent (\eg a list of web search results). The agent experiences this action \gaedit{and the resulting utility from this action is observed by the principal.} \gadelete{and reports a reward.} Each principal only observes its own users. Principals commit to their strategies in advance, so as to maximize their market share.
...
We revisit algorithms' performance \gaedit{in a standalone bandit problem, \ie without competition, and} we find that the most natural performance measure does not explain this phenomenon. We suggest a new, more nuanced performance measure that does.
