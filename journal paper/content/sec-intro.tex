...
Our model speaks to the ongoing debate on the role that data plays in the digital economy. One fundamental question in this debate is whether data can serve a similar role as traditional network effects, whereby competition \emph{for} the market is more important than competition \emph{in} the market, due to the winner-take-all nature of \asedit{the digital economy} \citep{Rysman09, jullien2019economics}. 
The ``death spiral" effect that we find has a similar flavor: 
\asdelete{as this as}
once a firm engages in exploration it gets completely starved of users. We further find that a small data advantage for one firm gets amplified under competition and leads to that firm taking the entire market. A distinctive feature of our approach is that we explicitly model the learning problem of the firms and consider them deploying algorithms for solving this problem.  Thus, we do not explicitly model \asedit{the} network effects, but they arise endogenously from our setup.
