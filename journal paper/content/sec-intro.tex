...
We draw on the machine learning literature to compare bandit algorithms to one another in the absence of competition, so that we can talk about \emph{better} bandit algorithms in a principled way.
...
\xhdr{Significance.}
\asedit{This paper paper connects two huge literatures: on regret-minimizing bandits in computer science and on competition in economics, while staying true to each.}
