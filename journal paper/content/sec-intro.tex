...
We investigate these questions for two separate but connected stylized duopoly models where two firms commit to exploration strategies and compete for a stream of consumers. In the first, which we denote as the \textit{expectation choice model}, we suppose that consumers are well-informed about the firm's exploration strategies but do not observe any information about past user experiences with the firms. In the second, which we denote as the \textit{reputation choice model}, we suppose that consumers \textit{only} consider the average of past user experiences with each of the firms. In the expectation choice model we analytically characterize the equilibrium exploration strategies as we vary the degree of randomness in consumer decisions. In the reputation choice model we account for competition in a more direct way and utilize numerical simulations to consider asymmetries in the timing of entry, increasing competition beyond two firms, and allow us to isolate the effect that role that informational advantages can play as a barrier to entry in markets where exploration and competition tensions arise.
...
\item In the expectation choice model, agents do not receive any other information and choose between the principals using their knowledge of $t$ and the principals' algorithms
\item In the reputation choice model, agents have access to a reputation score for each principal, which is a sliding window average of the rewards experienced by previous agents that have visited this principal
\end{enumerate}
}
\end{itemize}
