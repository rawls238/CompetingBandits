...
\item We investigate the algorithms' performance ``in isolation", \ie without competition. We suggest a new performance measure to explain why the greedy algorithm is sometimes not the best strategy under high levels of competition.%
    \footnote{Note the discrepancy with our theoretical results on \HardMax: there, the greedy algorithm is always the best strategy, mainly because it is aware of the Bayesian prior (whereas in the simulations the prior is not available).}
...
\xhdr{Economic interpretation: data network effects.}
\gaedit{Our model speaks to the ongoing debate of the role that data plays as a competitive advantage in the digital economy. One fundamental question in this debate is whether data serves as a network effect, whereby competition for the market is more important than competition in the market due to the winner-take-all nature of such markets \citep{farrell2007coordination, jullien2019economics}. We find a similar effect in our setting where slight asymmetries between the firms leads to a winner-take-all outcome. Indeed, we find that a small data advantage for one firm gets amplified under competition and leads to that firm taking the entire market.

A distinctive feature of our approach is that we explicitly model the learning problem of the firms and consider them as deploying algorithms for solving such problems. Our results highlight that understanding the performance of learning algorithms in isolation does not necessary translate to understanding their impact in competition, precisely due to the fact that competition leads to the endogenous generation of data observed by the firms. Thus, approaches such as \cite{bajari2018impact} that argue that the diminishing returns to scale and scope of data in isolation render such data feedback loops as non-existent ignore the differences induced by learning in isolation versus under competition. Furthermore, explicitly incorporating the interaction between learning technology and data creation allows us to speak on how data advantages are characterized and amplified not just by data quantity, but also the increased data quality gathered by better learning algorithms.
}
