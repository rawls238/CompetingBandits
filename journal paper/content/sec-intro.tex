...
\footnote{\asedit{As opposed to ``in competition": as solutions to a stand-alone exploration problem which aims to maximize the total consumer welfare over time (see Appendix~\ref{app:bg} for self-contained background).}}
...
One baseline is algorithms that do not explicitly explore (\emph{greedy algorithms}); \asedit{in isolation}, they are known to \gaedit{perform poorly}\gadelete{be terrible} for a wide variety of problem instances.
...
\gaedit{We recover a similar relationship even though our focus differs from the previous literature by analyzing data-based product innovation, which crucially depends on data that is generated by the consumers of the firm. We interpret the adoption of better exploration algorithms as innovation and control the severity of the competition by varying the agents' decision rule from \HardMax (cut-throat competition) to \HardMaxRandom to \SoftMaxRandom and all the way to the uniform selection.\footnote{Adoption of exploration algorithms tends to require substantial R\&D effort in practice, even if the algorithms are well-known and/or similar technologies already exist elsewhere  \citep[\eg see][]{DS-arxiv}.}$^{,}$\gaedit{\footnote{\gaedit{This interpretation is what allows us to connect to the regret-minimizing bandit literature in order to discuss innovation in a principled manner by providing us with a disciplined comparison between the algorithms selected by the firm.}}}}
...
\gaedit{Unlike traditional innovation models which study lab-based innovation, our focus on data-based innovation generates the traditional inverted-U relationship purely through the reputational consequences of exploration in competition, without explicitly invoking monetary costs and benefits. This is in contrast to the inverted-U relationships established in prior work which rely on monetary aspects: how much can a firm invest into  R\&D and how much can it profit from the innovation.}
...
%as non-existent
but ignore the differences induced by learning in isolation versus under competition. Furthermore, explicitly modeling the interaction between learning technology and data creation allows us to speak on how data advantages are characterized and amplified by the increased \emph{quality} of data gathered by better learning algorithms, not just the quantity thereof. \gaedit{Indeed, by exploring the first-mover advantage channel, we show how such incumbency may actually be good for innovation, but also lead to a barrier to entry precisely due to this feedback loop and how it's impacted by competition.}
