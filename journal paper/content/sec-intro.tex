...
\gaedit{Our model speaks to the ongoing debate on the role that data plays in the digital economy. One fundamental question in this debate is whether data can serve a similar role as traditional network effects, whereby competition for the market is more important than competition in the market due to the winner-take-all nature of such markets \citep{Rysman09, jullien2019economics}. The ``death spiral" effect that we find has a similar flavor as this as once a firm engages in exploration it gets completely starved of users. Indeed, we further find that a small data advantage for one firm gets amplified under competition and leads to that firm taking the entire market. A distinctive feature of our approach is that we explicitly model the learning problem of the firms and consider them as deploying algorithms for solving this problem.  Thus, we do not explicitly model such network effects, but rather they arise endogenously from our setup. 

Our results highlight that understanding the performance of learning algorithms in isolation does not necessarily translate to understanding their impact in competition, precisely due to the fact that competition leads to the endogenous generation of data observed by the firms. Approaches such as \cite{bajari2018impact} that argue that the diminishing returns to scale and scope of data in isolation render such data feedback loops as non-existent ignore the differences induced by learning in isolation versus under competition. Furthermore, explicitly incorporating the interaction between learning technology and data creation allows us to speak on how data advantages are characterized and amplified not only by data quantity, but also the increased data quality gathered by better learning algorithms.
}
